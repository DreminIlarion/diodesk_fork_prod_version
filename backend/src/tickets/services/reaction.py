...
class ReactionService:
    ...
    async def toggle(
        ...
        existing = await self.reaction_repo.find(
            comment_id=comment_id, author_id=current_subject.id, reaction_type=reaction_type
        )
        ...
        if existing is None:
            ...
            # Пользователь нажал на другую реакцию меняем тип
            existing.toggle(reaction_type, current_subject.id)
