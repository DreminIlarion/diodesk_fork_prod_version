...
// ==================== TASKS / ЗАДАЧИ ====================
export type TaskPriority = 'low' | 'medium' | 'high' | 'critical'; // ← английские!

export const TASK_PRIORITY_LIST: TaskPriority[] = ['low', 'medium', 'high', 'critical'];
...
type TaskStatus = 'backlog' | 'todo' | 'in_progress' | 'paused' | 'blocked' | 'to_review' | 'to_fix' | 'to_test' | 'done' | 'cancelled';
...
export const TASK_STATUS_COLORS: Record<TaskStatus, string> = {
  ...
  paused: 'bg-[var(--hover-2)] text-[var(--text-primary)]/60 border-[var(--border-color)]',
  blocked: 'bg-[var(--accent-soft)] text-[var(--accent)] border-[var(--accent)]/15',
  to_review: 'bg-violet-500/15 text-violet-400 border-violet-500/30',
  to_fix: 'bg-orange-500/15 text-orange-400 border-orange-500/30',
  to_test: 'bg-blue-500/15 text-blue-400 border-blue-500/30',
...
export type TaskReviewDecision = 'done' | 'to_fix' | 'to_test';
