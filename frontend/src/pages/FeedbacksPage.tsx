...
function StarRating({
  ...
  return (
    <div className="inline-flex flex-col items-start">
        ...
        {[1, 2, 3, 4, 5].map(star => {
          ...
          return (
            <button
              ...
              className={`${btnSizeMap[size]} flex items-center justify-center rounded-md transition-colors`}
            >
              <Star
                className={`${sizeMap[size]} flex-shrink-0 transition-colors ${
                  filled ? 'text-emerald-500 fill-emerald-500' : 'text-[var(--text-primary)]/15'
      ...
      {showLabel && label && (
        <span className="mt-1.5 text-xs font-medium text-emerald-500/75">
          {label}
        </span>
      )}
    </div>
        ...
        {!loading && options.map(opt => {
          ...
          return (
            <button
              key={opt.id}
              onClick={() => {
                onChange(opt.id, optionLabel);
                setOpen(false);
              }}
              className={`w-full flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm transition-colors
                ${value === opt.id ? 'bg-emerald-500/8 text-[var(--text-primary)]' : 'text-[var(--text-primary)]/60 hover:bg-[var(--hover-2)]'}`}
            >
        ...
        {!loading && options.map((u: any) => {
          ...
          return (
            <button
              key={u.id}
              onClick={() => {
                onChange(u.id, userLabel);
                setOpen(false);
              }}
              className={`w-full flex items-center gap-2 px-3 py-2.5 rounded-lg text-sm transition-colors
                ${value === u.id ? 'bg-emerald-500/8 text-[var(--text-primary)]' : 'text-[var(--text-primary)]/60 hover:bg-[var(--hover-2)]'}`}
            >
              ...
              <div className="flex-1 min-w-0 text-left">
                ...
                <div className="truncate text-[11px] text-[var(--text-primary)]/28">
                  {u.email}
                </div>
              </div>
  ...
  return (
    <div ref={triggerRef} className="relative">
      <div
        ...
        className={`w-full flex items-center gap-2.5 px-3.5 py-2.5 bg-[var(--hover-2)] border rounded-xl text-sm text-left transition-all select-none
          ${open ? 'border-[var(--accent)]/30 ring-2 ring-[var(--accent-ring)]' : 'border-[var(--border-color)]'}
          cursor-pointer hover:bg-[var(--hover-3)]`}
      >
...
function CreateFeedbackModal({
  ...
  useEffect(() => {
    if (presetTicketId) setTicketId(presetTicketId);
  }, [presetTicketId]);

  useEffect(() => {
    if (presetTicketLabel) setTicketLabel(presetTicketLabel);
  }, [presetTicketLabel]);
      ...
      >
        ...
        <div className="flex border-t border-[var(--border-color)]">
          <button
            onClick={onClose}
            disabled={deleting}
            className="flex-1 py-3 text-sm text-[var(--text-primary)]/60 hover:bg-[var(--hover-2)] disabled:opacity-50"
          >
            Отмена
          </button>
          <button
            onClick={confirm}
            disabled={deleting}
            className="flex-1 py-3 text-sm font-medium text-red-400 hover:bg-red-500/8 border-l border-[var(--border-color)] disabled:opacity-50"
          >
...
function FeedbackCard({
  ...
  return (
    <motion.div
      ...
      className="group bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl p-4 transition-all"
    ...
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex items-center gap-2.5 min-w-0">
          {author?.avatar_url ? (
            ...
            <div className="w-8 h-8 rounded-full bg-[var(--accent)] flex items-center justify-center text-white font-bold text-[10px] flex-shrink-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[11px] text-[var(--text-primary)]/28 flex items-center gap-1">
        ...
        {canManage && (
          <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
            <button
              ...
              className="p-1.5 rounded-lg hover:bg-[var(--hover-2)] text-[var(--text-primary)]/22 hover:text-[var(--text-primary)]/55"
            >
              <Pencil className="w-3.5 h-3.5" />
            </button>
            <button
              onClick={() => onDelete(feedback)}
              className="p-1.5 rounded-lg hover:bg-red-500/8 text-[var(--text-primary)]/22 hover:text-red-400"
            >
      ...
      <div className="flex items-center gap-2.5 mb-2.5">
        ...
        <span className="text-xs text-[var(--text-primary)]/38 font-medium">
          {RATING_LABELS[feedback.rating]}
        </span>
      </div>

      {feedback.comment && (
        <p className="text-sm text-[var(--text-primary)]/62 leading-relaxed mb-3 whitespace-pre-wrap">
          {feedback.comment}
        </p>
      )}
...
export default function FeedbacksPage() {
  ...
  const loadUsers = useCallback(async () => {
    try {
      ...
      if (filterAuthorId) {
        ...
        if (selected) {
          setFilterAuthorLabel(String(selected.full_name || selected.username || selected.email));
        }
      }
    } catch { }
  }, [filterAuthorId]);
  ...
  /* ─── Stats helper: load all feedbacks by pages ─── */
  const loadAllFeedbacksForStats = useCallback(async (filters: {
    ticketId?: string;
    author_id?: string;
  }) => {
    let currentPage = 1;
    const size = 100;
    let allItems: Feedback[] = [];
    let hasNext = true;
  ...
  const fetchFeedbacks = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const res = await feedbacksApi.getAll(page, pageSize, filters);
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <div className="relative">
            ...
            >
              ...
              {filterRating ? `${filterRating} ★` : 'Оценка'}
            </button>
                      ...
                      <div className="flex items-center gap-0.5 flex-shrink-0">
                        {Array.from({ length: 5 }, (_, i) => (
                          <Star
                            key={i}
                            className={`w-3 h-3 ${
                              i < r ? 'text-emerald-500 fill-emerald-500' : 'text-[var(--text-primary)]/10'
                            }`}
                          />
                        ))}
                      </div>
          ...
          <select
            ...
            className="px-3 py-2 rounded-xl bg-[var(--hover-2)] border border-[var(--border-color)] text-sm text-[var(--text-primary)]/60 focus:outline-none"
          >
          ...
          <button
            ...
            className="px-3 py-2 rounded-xl bg-[var(--hover-2)] border border-[var(--border-color)] text-sm text-[var(--text-primary)]/45 hover:text-[var(--text-primary)]/65"
          >
            Сбросить
          </button>
      ...
      {!loading && stats.total > 0 && (
        <div className="grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-4">
          <div
            ...
            style={{ boxShadow: 'var(--shadow-sm)' }}
          >
            <p className="text-[11px] uppercase tracking-wider text-[var(--text-primary)]/30 mb-3">
              Средняя оценка
            </p>
        ...
        <div className="flex items-center justify-center py-20">
          <div className="text-center max-w-xs">
            ...
            <p className="text-sm text-[var(--text-primary)]/30 mb-4">
              Попробуйте изменить фильтры или оставить новый отзыв
            </p>
