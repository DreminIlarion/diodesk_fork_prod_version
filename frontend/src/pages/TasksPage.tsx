...
function statusErr(err: any, task: TaskViewItem, to: TaskStatus) {
  ...
  return {
    title: `Ошибка перевода в «${ST_LABEL[to]}»`,
    description: raw,
  };
}
...
function SelectDD({
  ...
  useEffect(() => {
    ...
    const h = (e: MouseEvent) => {
      if (
        !tRef.current?.contains(e.target as Node) &&
        !dRef.current?.contains(e.target as Node)
      ) {
        setOpen(false);
      }
    };
...
function TCard({
  ...
  return (
    <motion.div
      ...
      onDragEnd={onDE}
      onClick={() => onView(t)}
      className={`group bg-[var(--bg-card)] border rounded-xl p-3.5 cursor-pointer transition-all hover:bg-[var(--hover-1)] hover:border-[var(--accent)]/30 shadow-sm ${dragging ? 'opacity-40 rotate-1 scale-105 shadow-xl' : ''
    ...
    >
      ...
      {(t.estimated_hours != null || t.actual_hours != null) && (
        <div className="flex flex-wrap gap-1.5 mb-2.5">
          {t.estimated_hours != null && <HoursBadge label="План" value={t.estimated_hours} />}
...
function KCol({
  ...
  return (
    <div
      ...
      className={`bg-[var(--hover-1)] rounded-xl flex flex-col w-[320px] shrink-0 border transition-all h-full ${isDO ? 'border-[var(--accent)] bg-[var(--accent)]/5 scale-[1.01]' : 'border-[var(--border-color)]'
        }`}
    >
...
function TaskEditorModal({
  ...
  const [startStatus, setStartStatus] = useState<'backlog' | 'todo'>(
    initSt === 'todo' ? 'todo' : 'backlog',
  );
      ...
      >
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-5">
          <div className="grid lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              {!lockTicket && (
                <div>
                  ...
                  <p className="mt-1 text-xs text-[var(--text-primary)]/35">
                    Если у задачи есть связь с тикетом — она откроется по номеру тикета.
                  </p>
                </div>
              )}
              ...
              {mode === 'create' && (
                <div className="rounded-2xl border border-[var(--border-color)] p-4 bg-[var(--hover-1)]">
                  <div className="text-sm font-medium text-[var(--text-primary)] mb-2">После создания</div>

                  <div className="flex flex-col sm:flex-row gap-2">
                    <button
                      onClick={() => setStartStatus('backlog')}
                      className={`flex-1 px-4 py-3 rounded-xl text-sm border transition-all ${startStatus === 'backlog'
                          ? 'bg-[var(--accent)]/10 text-[var(--accent)] border-[var(--accent)]/30'
                          : 'bg-[var(--bg-card)] text-[var(--text-primary)]/55 border-[var(--border-color)] hover:bg-[var(--hover-2)]'
                        }`}
                    >
                      Оставить в «{ST_LABEL.backlog}»
                    </button>

                    <button
                      onClick={() => setStartStatus('todo')}
                      className={`flex-1 px-4 py-3 rounded-xl text-sm border transition-all ${startStatus === 'todo'
                          ? 'bg-blue-500/10 text-blue-400 border-blue-500/30'
                          : 'bg-[var(--bg-card)] text-[var(--text-primary)]/55 border-[var(--border-color)] hover:bg-[var(--hover-2)]'
                        }`}
                    >
                      Сразу в «{ST_LABEL.todo}»
                    </button>
                  </div>

                  {initSt && initSt !== 'backlog' && initSt !== 'todo' && (
                    <p className="mt-2 text-xs text-[var(--text-primary)]/40">
                      Из этой формы задача создаётся понятно и безопасно: сначала в бэклог/к выполнению,
                      потом её можно перетащить дальше по доске.
                    </p>
                  )}
                </div>
              )}
        ...
        <div className="flex justify-end gap-2.5 px-5 py-4 border-t border-[var(--border-color)] bg-[var(--hover-1)] shrink-0">
          <button
            onClick={() => !saving && onClose()}
            disabled={saving}
            className="px-5 py-2.5 rounded-xl bg-[var(--hover-2)] text-[var(--text-primary)]/70 font-medium hover:bg-[var(--hover-3)] disabled:opacity-50 text-sm"
          >
            Отмена
          </button>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-5 space-y-4">
            ...
            <div className="rounded-xl border border-[var(--border-color)] overflow-hidden">
              ...
              >
                <span className="flex items-center gap-2">
                  ...
                  {t.assignee_id ? 'Сменить исполнителя' : 'Назначить исполнителя'}
                </span>
          ...
          {canReview && (
            <div className="flex gap-3">
              <button
                onClick={() => act('rv', () => tasksApi.review(t.id, { decision: 'done' }), 'Задача принята')}
                disabled={busy === 'rv'}
          ...
          {ticketPath && (
            <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-[var(--hover-1)] border border-[var(--border-color)]">
              ...
              <Link
                to={ticketPath}
                onClick={onClose}
                className="text-sm text-[var(--accent)] flex items-center gap-1 font-medium hover:underline"
              >
                Открыть
